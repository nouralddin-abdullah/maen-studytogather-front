...
function EditRoomModal({ isOpen, onClose, room }) {
  ...
  const removeWallpaper = () => {
    setValue("wallpaper", undefined, { shouldValidate: true, shouldDirty: true });
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      const message = err.response?.data?.message || err.message || "فصل تحديث الغرفة. حاول مرة أخرى.";
      toast.error(message);
    } finally {
            ...
            <div className="space-y-1.5">
              ...
              {wallpaperPreview ? (
                ...
                >
                  ...
                  <span className="text-sm font-medium">اضغط لرفع صورة الخلفية</span>
                  <span className="text-xs text-text-muted">JPEG, PNG, WebP — حتى 5 ميجابايت</span>
                </button>
              )}
              ...
              />
              {errors.wallpaper && (
                <p className="text-xs text-error mt-1">{errors.wallpaper.message}</p>
              )}
            </div>
